...
- **display**, optional, relation 3 and above only: how the anagram should read on the site's Discover page, when its words read better with a listed contraction, punctuation or capitals: `I'm a dirty room.` for the words `im a dirty room`. The same words, in the same order, once each, as themselves or as one of the site's listed forms (`it's`, `don't`, `I'm`, `you're`, `that's`, `o'clock`, and the like: a spelling with an apostrophe whose letters are the word); an apostrophe, hyphen, comma, full stop, question mark, colon, semicolon or quotation marks; capitals only for I, names, acronyms and the start of a sentence. Never an exclamation mark, never a possessive (`dog's` is not a form of `dogs`), never a word changed, added or left out. Most anagrams need none. A display that breaks the rule is left off and the verdict kept.
...
A line whose anagram reads better with a listed contraction, punctuation or capitals may add `"display": "<the words as they should read>"`; for "im a dirty room", `"display": "I'm a dirty room."`.
