...
When you want to decide, across everything already published, which anagrams are Greatest Hits, which are
only Interesting, and which should come off the page. The review desk handles what a queue adds; the audit
handles what the page already shows.
...
2. Every published anagram is listed under the label it has now: **Greatest Hits** (`featured`) first,
   then **Interesting** (`accepted`). Within each, the anagrams the judge suggested for Greatest Hits come
   first, then the strongest links. Filter by text or category, or show only the suggestions or what you
   have changed.
3. Change a row's label to Greatest Hits, Interesting or **Remove from the page** (`retired`), and edit its
...
| Interesting | `pnpm hits:set --status=accepted id…` |
