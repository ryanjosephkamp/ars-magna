...
`hits:input` changes how a hit's input reads when its letters stay exactly the same, in the same order. A
hit from before phase N was made with its digits dropped, so for it "Big Brother 28" and "Big Brother" are
one input with one id:
