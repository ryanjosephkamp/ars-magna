...
Review Greatest Hits pull request pr_number. Read AGENTS.md, then follow "Review a routine pull request" in docs/OPERATOR.md. First change nothing. Check out the branch and list every hit it adds, by shelf, with its id, relation, reads, input, phrase and justification. Then list its alternates and the report's near misses, the sentences its About section says reached a hit, marking the ones the judge wrote, and every sense its Senses section lists, with the dictionary gloss it is read over. Mark any hit tagged tone:rude. For each shelved hit, recommend one of: keep it where it is, promote it to featured, hold it back as proposed, or retire it. For each sense, say whether it reads the word as the anagram does and stays a reading rather than a fact about the input. For each alternate, recommend accepting it or leaving it. Give one reason each, then wait for my answer. When I name ids and statuses, run pnpm hits:set with exactly those, and pnpm hits:sense for any sense I name to change or clear, commit data/hits.jsonl on the branch, push, report CI, and stop. I merge.
