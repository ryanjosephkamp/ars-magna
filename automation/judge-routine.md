...
6. Push the branch and open a pull request against `main` with the same title. The body is the ingest report, which says what merging accepts, followed by one line naming the model that judged and the rubric version. If N is zero, still open the pull request: the judged queue is part of the record, and a person should see that nothing cleared the bar.
