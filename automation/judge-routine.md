...
7. Read `data/queue/<date>/ingest-report.md`. Create a branch named `hits/<date>`, commit `data/hits.jsonl`, `data/candidates.jsonl` and everything in `data/queue/<date>/` except `raw.jsonl` and `judge-input-*.md`, with the message `Greatest Hits: <N> new for <date>`, where N is the number of new hits the report gives.
