...
| desk | `pnpm hits:desk` | Builds the review desk, one self-contained page in `.cache/desk/index.html`, from the hits, the candidates and the newest judged queues. It writes nothing to the repository; each decision in it becomes one of these commands, and it fills `docs/prompts/apply-desk.md` with them. | a person · laptop |
