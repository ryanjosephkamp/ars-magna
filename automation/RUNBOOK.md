...
| ingest | `pnpm hits:ingest --model=…` | Validates the verdicts and re-checks the letters. Shelves rubric v2 verdicts: up to three `accepted` hits per input, further qualifying rows as `proposed` alternates, and near misses left in the verdicts. Rubric v1 verdicts from older queues become `proposed` at a total of 11. Keeps the judge's `about` for an input that has none and copies each input's sentence onto its hits. Moves candidates to `enumerated` and writes `ingest-report.md`. With `--only=id,… --status=accepted\|proposed` it writes only those rows from the queue's verdicts, past the shelves, for promoting a near miss, and changes neither the candidates nor the report. | Claude routine · laptop |
