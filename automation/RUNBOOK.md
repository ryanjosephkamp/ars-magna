...
| judge | `pnpm hits:judge` | For a screened queue, checks `screen-output.jsonl` against the screen input and rebuilds the kept phrases into `screened.jsonl`. Then writes `judge-input-N.md` for a Claude session to answer into `judge-output.jsonl`. `--via=api` calls the Claude API for the judging instead. A queue with nothing to judge gets an empty answer, not an error. | Claude routine · laptop |
...
| desk | `pnpm hits:desk` | Builds the review desk, one self-contained page in `.cache/desk/index.html`, from the hits, the candidates and the newest judged queues. It writes nothing to the repository; each decision in it becomes one of these commands, and it fills `docs/prompts/apply-desk.md` with them. With `--audit` it builds the Greatest Hits audit into `.cache/desk/audit.html` instead: every published hit under its label, Greatest Hits or Interesting, to relabel or remove. | a person · laptop |
