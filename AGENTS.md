...
| `apps/web` | the site; `hits.html` is the Discover page (Greatest Hits, Interesting, A stretch), built from `data/hits.jsonl`; `build.html` is the Build page (`src/build/`); `scripts/` writes `public/hits.json` and `public/terms.json` (the class files' terms) before every dev run and build |
| `apps/web/src/lib` | pure modules the components lean on: `orderings.ts`, `chosen.ts`, `share.ts`, `urlState.ts`, `resultView.ts`, `exporters.ts`, the term classes' `classes.ts`, `terms.ts`, `rowTerms.ts` and `readingChoice.ts`, and Build's `ledger.ts`, `checks.ts`, `analysis.ts`, `letterChart.ts`, `letterMap.ts` and `textCount.ts` |
